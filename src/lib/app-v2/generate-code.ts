...
export const writeModelsDTsFile = async (
  ...
  const relPath = join(options.outputDirectory, MODELS_D_FILENAME);
  const absPath = join(options.outputDirectoryAbsolutePath, MODELS_D_FILENAME);
  const decls = parsedSchema.models
    .flatMap((m) => {
...
export const writeDatabaseJsFile = async (
  ...
  const relPath = join(options.outputDirectory, DATABASE_FILENAME);
  const absPath = join(options.outputDirectoryAbsolutePath, DATABASE_FILENAME);
