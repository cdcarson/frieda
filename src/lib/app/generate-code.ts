...
import ora from 'ora';
export const generateCode = async (options: Options, schema: ParsedSchema): Promise<string[]> => {
  ...
  modelsDCode.code = prettier.format(modelsDCode.code, {...options.prettierOptions, filepath: modelsDCode.filePath})
    ...
    return {
      code: prettier.format(o.code, {...options.prettierOptions, filepath: o.filePath}),
      filePath: o.filePath
    }
  });
  if (options.compileJs) {
    ...
    jsFiles.push(modelsDCode);
    filesToWrite = [...jsFiles]

  } else {
    filesToWrite = [...otherTsTfiles, modelsDCode]
  }
...
export const getModelsDSourceCode = (schema: ParsedSchema, bannerComment: string): string => {
	...
	for (const f of model.fields) {
		if (f.isGeneratedAlways) {
			...
			if (f.isAutoIncrement) {
				notes.push(`${f.fieldName} is **optional** (column is \`auto_increment\`)`);
			} else if (f.hasDefault) {
				notes.push(`${f.fieldName} is **optional** (column has a default value)`);
			}
...
export const getFindUniqueTypeDeclaration = (model: ParsedModel): string => {
	...
	const declaration = `export type ${model.findUniqueTypeName}=${uniqueTypes.join('|')}`;
	return [comment, declaration].join('\n');
};
...
export const getDatabaseSourceCode = (schema: ParsedSchema, bannerComment: string): string => {
	const code = `
  ${bannerComment}
...
export const getSearchIndexesSourceCode = (schema: ParsedSchema, bannerComment: string): string => {
	const map: { [key: string]: FullTextSearchIndex } = schema.models.reduce((acc, m) => {
		const copy = { ...acc };
...
export const getSchemaSourceCode = (schema: ParsedSchema, bannerComment: string): string => {
