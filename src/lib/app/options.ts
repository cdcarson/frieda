...
import { DEFAULT_PRETTIER_OPTIONS, ENV_DB_URL_KEYS, FRIEDA_RC_FILE_NAME } from './constants.js';
...
export class Options {
	...
	static async create(cwd: string, cliArgs: Partial<FriedaCliArgs>): Promise<Options> {
	...
	async init() {
		...
		this.#prettierOptions = (await prettier.resolveConfig(this.cwd)) || DEFAULT_PRETTIER_OPTIONS;
		...
		const envFile =
			typeof this.cliArgs.envFile === 'string' && this.cliArgs.envFile.length > 0
		...
		let outputDirectory =
			typeof this.cliArgs.outputDirectory === 'string' && this.cliArgs.outputDirectory.length > 0
				? this.cliArgs.outputDirectory
				: typeof rcOptions.outputDirectory === 'string' && rcOptions.outputDirectory.length > 0
      ...
      log.info(squishWords(OPTION_DESCRIPTIONS.outputDirectory).split('\n'))
		...
		if (changed) {
			...
			if (answers.saveChanges) {
				...
				rcOptions = { ...rcOptions, outputDirectory, envFile: databaseOptions.envFile, compileJs };
				await fs.writeFile(
	...
	async validateEnvFile(relPath: string): Promise<DatabaseOptions> {
		...
		const foundKeys = ENV_DB_URL_KEYS.filter((k) => envKeys.includes(k) && env[k].length > 0);
		if (foundKeys.length === 0) {
			throw new Error(`Could not find either ${ENV_DB_URL_KEYS.join(' or ')} in ${relPath}.`);
		}
	...
	async promptOutputDirectory(currentValue: string, rcValue?: string): Promise<string> {
		const answers = await prompts({
			name: 'relPath',
			type: 'text',
	...
	async validateOutputDirectory(relPath: string): Promise<string> {
		...
		if (!absolutePath.startsWith(this.cwd) || absolutePath === this.cwd) {
			throw new Error(`The output directory must a subdirectory of the current working directory.`);
		}
