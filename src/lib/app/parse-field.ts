...
import { MYSQL_TYPES, type CastType, type MysqlBaseType } from '../api/types.js';
...
export const parseField = (column: ColumnRow): ParsedField => {
	...
	const isGeneratedAlways = /\b(VIRTUAL|STORED) GENERATED\b/i.test(column.Extra);
	const isTinyIntOne =
		'tinyint' === mysqlBaseType && getParenthesizedArgs(column.Type, 'tinyint').trim() === '1';
	const isInvisible = /\bINVISIBLE\b/i.test(column.Extra);
	const annotationRx = /(?:^|\s+)@(bigint|set|json)(?:\((.*)\))?/gi;
	const typeAnnotations: ParsedAnnotation[] = Array.from(column.Comment.matchAll(annotationRx)).map(
		(r) => {
			return {
	...
	const bigIntAnnotation =
		mysqlBaseType === 'bigint' ? typeAnnotations.find((a) => a.annotation === 'bigint') : undefined;
	const setAnnotation =
		mysqlBaseType === 'set' ? typeAnnotations.find((a) => a.annotation === 'set') : undefined;
	let jsonAnnotation: Required<ParsedAnnotation> | undefined =
		mysqlBaseType === 'json'
			? (typeAnnotations.find((a) => a.annotation === 'json') as Required<ParsedAnnotation>)
			: undefined;
	if (jsonAnnotation) {
		if (
	...
	if ('json' === castType) {
		javascriptType = jsonAnnotation ? jsonAnnotation.typeArgument : DEFAULT_JSON_FIELD_TYPE;
	} else if ('set' === castType) {
		javascriptType = setAnnotation ? `Set<${jsEnumerableStringType}>` : 'string';
