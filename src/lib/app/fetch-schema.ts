...
export const fetchTableNames = async (connection: Connection): Promise<FetchTableNamesResult> => {
...
export const fetchSchema = async (connection: Connection): Promise<FetchedSchema> => {
  const spinner = ora('Fetching schema...').start();
	const { databaseName, tableNames } = await fetchTableNames(connection);
	const tables = await Promise.all(tableNames.map((t) => fetchTable(connection, t)));
