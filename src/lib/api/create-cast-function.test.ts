...
describe('createCastFunction', () => {
  ...
  it('date in schema', () => {
    ...
    const fn = createCastFunction(schemaCast);
    expect(
      fn({ ...f, orgTable: 't', orgName: 'a' }, new Date().toISOString())
    ).toEqual(expect.any(Date));
  });
  it('date in custom', () => {
    const fn = createCastFunction(schemaCast, { c: 'date' });
    expect(fn({ ...f, name: 'c' }, new Date().toISOString())).toEqual(
      expect.any(Date)
    );
  });
  it('float in schema', () => {
  ...
  it('json in schema', () => {
    ...
    const fn = createCastFunction(schemaCast);
    expect(
      fn({ ...f, orgTable: 't', orgName: 'a' }, JSON.stringify({ a: 8 }))
    ).toEqual({ a: 8 });
  });
  it('set in schema', () => {
    schemaCast['t.a'] = 'set';
    const fn = createCastFunction(schemaCast);
    expect(fn({ ...f, orgTable: 't', orgName: 'a' }, 'a,b')).toEqual(
      expect.any(Set)
    );
  });
