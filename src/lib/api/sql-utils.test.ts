...
describe('getWhere', () => {
  it('model where', () => {
    ...
    expect(where.sql).toEqual(expect.stringMatching(/^WHERE\s+`a`\s+=\s+\?\s+AND\s+`b`\s+=\s+\?/))
    expect(where.values).toEqual(['foo', 'bar'])
  })
  it('model where with table name', () => {
    const where = getWhere({a: 'foo', b: 'bar'}, 't');
    expect(where.sql).toEqual(expect.stringMatching(/^WHERE\s+`t`.`a`\s+=\s+\?\s+AND\s+`t`.`b`\s+=\s+\?/))
    expect(where.values).toEqual(['foo', 'bar'])
  })
...
describe('getOrderBy', () => {
  ...
  it('model order by w table', () => {
    expect(getOrderBy({col: 'a', dir: 'desc'}, 't').sql).toBe('ORDER BY `t`.`a` desc')
  })
  it('model order by is array', () => {
    expect(getOrderBy([{col: 'a', dir: 'desc'}]).sql).toBe('ORDER BY `a` desc')
  })
  it('model with more than one order field', () => {
    expect(getOrderBy([{col: 'a', dir: 'desc'}, {col: 'b', dir: 'asc'}]).sql).toBe('ORDER BY `a` desc,`b` asc')
  })
