...
import { MYSQL_TYPES, type ColumnRow, type IField } from './types.js';
...
describe('Field', () => {
  ...
  it('can be stringified', () => {
    ...
    const deser: IField = JSON.parse(ser);
    expect(deser.column).toEqual(f.column)
  ...
  it('bigIntAnnotation', () => {
    ...
    expect(
      new Field({ ...column}).bigIntAnnotation
    ).toBeUndefined();
  })
  it('setAnnotation', () => {
    expect(
      new Field({ ...column, Comment: '@set' }).setAnnotation
    ).toEqual(
      {
        fullAnnotation: '@set',
        annotation: 'set'
    ...
    expect(
      new Field({ ...column}).setAnnotation
    ).toBeUndefined();
  })
  ...
  it('jsonAnnotation', () => {
    ...
    expect(
      new Field({ ...column}).jsonAnnotation
    ).toBeUndefined();
  })
  ...
  it('enumAnnotation', () => {
    ...
    expect(
      new Field({ ...column}).enumAnnotation
    ).toBeUndefined();
  })

  it('isTinyIntOne', () => {
    expect(
      new Field({ ...column, Type: 'tinyint(1)' }).isTinyIntOne
    ).toBe(true)
    expect(
      new Field({ ...column, Type: 'int(1)' }).isTinyIntOne
    ).toBe(false)
    expect(
      new Field({ ...column, Type: 'tinyint' }).isTinyIntOne
    ).toBe(false)
  ...
  describe('castType', () => {
    ...
    it('tinyint(1)', () => {
      ...
      expect(new Field({ ...column, Type: 'tinyint(1)' }).castType).toBe('boolean');
    });

    it('bool and boolean', () => {
      expect(new Field({ ...column, Type: 'bool' }).castType).toBe('boolean');
      expect(new Field({ ...column, Type: 'boolean' }).castType).toBe('boolean');
    });
  ...
  describe('javascriptType', () => {
    it('json without a type', () => {
      expect(new Field({ ...column, Type: 'json' }).javascriptType).toBe('unknown');
      expect(
    ...
    it('bigint', () => {
      expect(new Field({ ...column, Type: 'bigint' }).javascriptType).toBe('string');
    });

    it('bigint with @bigint', () => {
      expect(
        new Field({ ...column, Type: 'bigint', Comment: '@bigint' }).javascriptType
      ).toBe('bigint');
    });
    ...
    it('bool and boolean', () => {
      expect(new Field({ ...column, Type: 'bool' }).javascriptType).toBe('boolean');
      expect(new Field({ ...column, Type: 'boolean' }).javascriptType).toBe('boolean');
    });
    it('set without annotation', () => {
      expect(
        new Field({ ...column, Type: `set('a','b')`, Comment: '' }).javascriptType
      ).toBe('string');
    });
    it('set with plain annotation', () => {
      expect(
        new Field({ ...column, Type: `set('a','b')`, Comment: '@Set' }).javascriptType
      ).toBe(`Set<'a'|'b'>`);
      expect(
    ...
    it('enum without annotation', () => {
      expect(
        new Field({ ...column, Type: `enum('a','b')`, Comment: '' }).javascriptType
      ).toBe(`'a'|'b'`);
      expect(
    ...
    it('tinyint with width other than 1', () => {
      ...
      expect(new Field({ ...column, Type: 'tinyint' }).javascriptType).toBe('number');
    });
    it('all the other int types', () => {
      ['tinyint', 'smallint', 'mediumint', 'int', 'integer', 'year'].forEach(
        (t) => {
